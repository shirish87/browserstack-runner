...
function readFiles(files, separator, callback) {
  ...
  if (!Array.isArray(files) || !fileCount) return callback();
...
function getPatch(framework, callback) {
  ...
  if (patchCache[framework]) return callback(null, patchCache[framework]);
  ...
  readFiles(frameworkFiles, '\n', function (err, contents) {
    ...
    contents = patchTagStart + contents + patchTagEnd;
    patchCache[framework] = contents;
