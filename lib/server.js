...
exports.Server = function Server(bsClient, workers) {
  ...
  function handleFile(filename, request, response, doNotUseProxy) {
    ...
    if (query._worker_key && workers[query._worker_key]) {
      ...
      worker.stopTimer('launch');
      worker._launched = true;

      if (!worker.isTimerActive('load') && !worker._loaded) {
        worker.startTimer('load');
      }
    }
  ...
  var handlers = {
    '_ack': function ackHandler(uri, body, request, response) {
      ...
      if (worker.isTimerActive('load')) {
        worker.stopTimer('load');
        worker._loaded = true;
      }
      ...
      bsClient.takeScreenshot(worker.id, function(error, screenshot) {
        ...
        checkAndTerminateWorker(worker, function(reusedWorker) {
          ...
          if (reusedWorker) {
            ...
            worker._launched = false;
            worker.resetTimer('launch');
