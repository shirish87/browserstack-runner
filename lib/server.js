...
exports.Server = function Server(bsClient, workers) {
  ...
  function handleFile(filename, request, response, doNotUseProxy) {
    ...
    var writeResponse = function(err, data) {
      ...
      inject.applyPatch(config.test_framework, data, function (err, content) {
        if (err || !content) {
          logger.info('Failed to apply patch:', err || 'no content');
        } else {
          data = content;
        }
    ...
    if (!doNotUseProxy && config.test_server) {
      proxyServer.onRequest(request, response, config.test_server, function(remote_response, response_data) {
        ...
        if (mimeType === 'text/html' && pathMatches) {
          inject.applyPatch(config.test_framework, response_data, function (err, content) {
            if (err || !content) {
              logger.info('Failed to apply patch:', err || 'no content');
            } else {
              response_data = content;
              headers['content-length'] = response_data.length;
            }
  ...
  var handlers = {
    ...
    '_log': function logHandler(uri, body, request, response) {
      ...
      var workerId = ((uuid && workers[uuid]) || {}).id;
      logger.trace('[%s] _log', workerId, query);
      ...
      if (query && Array.isArray(query.arguments)) {
        ...
        try {
          ...
          vm.runInNewContext('output = format.apply(null, input.map(function (s) { return eval(s); }));', context);
          logger.info('[' + request.headers['x-browser-string'] + '] ' + context.output);
          logged = true;
        } catch (e) {
          logger.trace('[%s] _log: failed to format console log data', workerId, query);
        }
      }
